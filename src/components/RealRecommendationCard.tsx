...
import { PRIORITY_STYLE, PriorityBadge, PillarBadge, type Priority } from "./recoBadges";

// Carte de recommandation — design VERBATIM de la maquette Barth (RecommendationCard),
// pilotée par NOS données réelles (lib/recos.Reco). Mapping :
//  - impact Fort/Moyen/Faible → priorité P1/P2/P3
//  - axe → pilier (badge coloré)
//  - objectif → titre (wording marché) ; diagnostic → observation (constat technique)
const IMPACT_TO_PRIORITY: Record<string, Priority> = {
  Fort: "P1",
  Moyen: "P2",
  Faible: "P3",
};

export default function RealRecommendationCard({
  rec,
  index,
  delay = 0,
}: {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <h3 className="text-[15px] font-medium leading-snug text-text-primary">{title}</h3>

        {/* Observation technique */}
        {observation && (
          <p className="text-[13px] font-light leading-relaxed text-text-secondary">{observation}</p>
        )}
