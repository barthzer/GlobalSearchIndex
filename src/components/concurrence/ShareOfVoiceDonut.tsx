...
export default function ShareOfVoiceDonut({ data }: Props) {
  ...
  // `total` (avec `|| 1`) = dénominateur des pourcentages (évite /0).
  const total = traffic.reduce((s, t) => s + t, 0) || 1;
  // Total AFFICHÉ = somme des clics par marque ARRONDIS → cohérent avec les lignes
  // (fini « TOTAL 1 » au-dessus de lignes à 0 : arrondi haut du brut vs arrondi bas
  // des lignes).
  const displayTotalClics = traffic.reduce((s, t) => s + Math.round(t), 0);
        ...
        <div className="relative shrink-0" style={{ width: size, height: size }}>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="mt-1 text-2xl font-bold tabular-nums text-text-primary">
              {focusIdx !== null ? `${shares[focusIdx].toFixed(1)}%` : fmt(displayTotalClics)}
            </span>
