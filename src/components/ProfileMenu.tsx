...
export default function ProfileMenu({ dropUp = false }: { dropUp?: boolean }) {
  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      {/* Dropdown */}
      <div
        className={`absolute right-0 ${dropUp ? "bottom-10 origin-bottom" : "top-10 origin-top"} w-64 overflow-hidden rounded-xl border border-border-subtle bg-modal-bg shadow-[0px_16px_40px_-8px_rgba(0,0,0,0.3)] transition-all duration-200`}
        style={{
          opacity: open ? 1 : 0,
          transform: open ? "translateY(0) scale(1)" : `translateY(${dropUp ? "4px" : "-4px"}) scale(0.97)`,
