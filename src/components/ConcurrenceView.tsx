...
import CoverageRadar from "./concurrence/CoverageRadar";
import CoverageBump from "./concurrence/CoverageBump";
import ShareOfVoiceDonut from "./concurrence/ShareOfVoiceDonut";
import { Brand, ConcurrenceData, Keyword, generateCoverageInsight } from "./concurrence/types";
...
const coverageTabs: { id: CoverageView; label: string; icon: React.ReactNode }[] = [
  ...
  },
  {
    ...
    icon: (
      <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        <path d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18Zm0 0v-3m0-3v-3m0-3V6m-6 6h3m3 0h3m3 0h3" />
      </svg>
    ),
  },
  {
...
export default function ConcurrenceView() {
      ...
      {!configured || !data ? (
        ...
        <>
          ...
          <section className="mb-6 rounded-2xl border border-border-subtle bg-bg-card backdrop-blur-[6px]">
            ...
            {/* Tabs */}
            <div className="flex flex-wrap gap-0 px-5 pt-6 md:px-6">
              {coverageTabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setCoverageView(tab.id)}
            ...
            >
              ...
              {coverageView === "radar" && <CoverageRadar data={data} />}
              {coverageView === "bump" && <CoverageBump data={data} />}
  ...
  return (
    <section className="rounded-2xl border border-border-subtle bg-bg-card backdrop-blur-[6px]">
      <div className="flex items-center gap-2 px-5 pt-5 md:px-6">
        ...
        <h3 className="text-[length:var(--text-body-lg)] font-medium text-text-heading">Couverture top 10</h3>
      </div>
      <div className="flex flex-wrap gap-0 px-5 pt-6 md:px-6">
        {coverageTabs.map((tab) => (
          <button
