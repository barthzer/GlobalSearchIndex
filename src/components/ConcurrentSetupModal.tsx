...
interface ConcurrentSetupModalProps {
  initialCompetitors?: string[];
  initialKeywords?: string[];
  onClose: () => void;
  onSubmit: (competitors: string[], keywords: string[]) => void;
}

export default function ConcurrentSetupModal({
  initialCompetitors,
  initialKeywords,
  onClose,
  onSubmit,
}: ConcurrentSetupModalProps) {
  const [step, setStep] = useState<1 | 2>(1);
  const [competitors, setCompetitors] = useState<string[]>(() => {
    const init = initialCompetitors ?? [];
    return [...init, ...Array(Math.max(0, 3 - init.length)).fill("")];
  });
  const [keywords, setKeywords] = useState<string[]>(() => {
    const init = initialKeywords ?? [];
    return [...init, ...Array(Math.max(0, 10 - init.length)).fill("")];
  });
  ...
  function handleStep1() {
    ...
    if (dup) {
      setError(`Concurrent en double : "${dup}". Chaque site doit être unique.`);
      return;
    }
    setError("");
    setStep(2);
  ...
  function handleStep2(e: React.FormEvent) {
    e.preventDefault();
    const filledCompetitors = competitors.map((c) => c.trim()).filter(Boolean);
    const filledKeywords = keywords.map((k) => k.trim()).filter(Boolean);
    if (filledKeywords.length < 5) {
      setError("Ajoutez au moins 5 mots-clés stratégiques.");
      return;
    }
    const normalized = filledKeywords.map((k) => k.toLowerCase());
    const dup = normalized.find((k, i) => normalized.indexOf(k) !== i);
    if (dup) {
      setError(`Mot-clé en double : "${dup}". Chaque mot-clé doit être unique.`);
      return;
    }
    onSubmit(filledCompetitors, filledKeywords);
  }
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      <div
        className="relative w-full max-w-[480px] rounded-[2rem] border border-white/[0.06] bg-input-bg p-2"
      ...
      >
        ...
        <div className="relative max-h-[85vh] overflow-y-auto rounded-[calc(2rem-0.5rem)] border border-border-subtle bg-modal-bg p-8 shadow-[inset_0_1px_1px_rgba(255,255,255,0.06)]">
          <div
            ...
            style={{
              background: "radial-gradient(circle, rgba(236,77,203,0.5) 0%, transparent 70%)",
            }}
          />
          ...
          {step === 1 ? (
            <div className="relative">
              <div className="mb-6 flex flex-col items-center text-center">
                ...
                <p className="text-[14px] font-extralight leading-relaxed text-text-secondary">
                  Renseignez jusqu&apos;à 3 sites concurrents pour comparer votre couverture SEO.
                </p>
              </div>
            ...
            <div className="relative" style={{ animation: "fade-up 300ms var(--ease-expo) both" }}>
              ...
              <div className="mb-6 flex flex-col items-center text-center">
                ...
                <p className="text-[14px] font-extralight leading-relaxed text-text-secondary">
                  Ajoutez entre 5 et 10 mots-clés sur lesquels vous souhaitez comparer votre présence.
                </p>
              </div>
              ...
              <form onSubmit={handleStep2} className="flex flex-col gap-3">
                {keywords.map((val, i) => (
                  <input
                    ...
                    placeholder={`Mot-clé ${i + 1}${i >= 5 ? " (optionnel)" : ""}`}
                    autoFocus={i === 0}
                    className={`w-full rounded-xl border bg-input-bg px-4 py-3 text-[14px] text-text-primary placeholder:text-text-muted outline-none transition-all duration-300 ${
                      error
                        ? "border-red-400/50"
