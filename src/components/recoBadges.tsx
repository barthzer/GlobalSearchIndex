// Style & badges des recommandations — repris VERBATIM de la maquette Barth
// (PriorityBadge en 3 barres, PillarBadge coloré + icône). Types définis LOCALEMENT
// (plus d'import des données mock rapport/recommendations — cf garde-fou check-mock-leak).
...
const PILLAR_FALLBACK = {
  chip: "border-border-subtle bg-card-inner-bg text-text-secondary",
...
/** Couleurs par priorité (P1 critique → P3 moyen) — index numéroté. */
...
/** Niveau de priorité en barres (rouge élevée, violet moyenne, bleu basse). */
...
/** Pastille de pilier colorée + icône. `pillar` libre (données réelles) → repli
 *  neutre si le pilier n'est pas un des 4 connus. */
export function PillarBadge({ pillar }: { pillar: string }) {
  const s = PILLAR_STYLE[pillar as Pillar] ?? PILLAR_FALLBACK;
  return (
