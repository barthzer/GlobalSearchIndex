...
import { stripWww } from "@/lib/domain";
...
function NonClasseChip({ reason }: { reason?: "growth_null" | "bas_floor" }) {
  const tooltip =
    reason === "bas_floor"
      ? "Autorité trop faible (indice ≤ 4), hors panel de comparaison."
      : reason === "growth_null"
        ? "Croissance des backlinks indisponible, non incluse au classement."
        : "Non classé.";
  return (
...
function CompositeCell({
  ...
  return (
    <div className="flex flex-col items-end gap-0.5">
      ...
      <span className="text-[10px] font-light tabular-nums text-text-muted">Indice {bas}</span>
    </div>
  );
}
...
function CompetitorsTable({
  ...
  return (
    <section className="rounded-2xl border border-border-subtle bg-bg-card p-5 backdrop-blur-[6px] md:p-6">
      ...
      {/* Distingue les deux périmètres concurrentiels (l'utilisateur ne peut pas le
          deviner) : ici l'autorité, l'onglet Concurrence la visibilité SERP. */}
      <p className="mb-3 text-[12px] font-light leading-relaxed text-text-muted">
        Panel d&apos;autorité (backlinks, médias). L&apos;onglet Concurrence compare la
        visibilité SERP sur vos mots-clés, son panel peut différer.
      </p>
          ...
          <thead>
            <tr>
              ...
              <th className="border-b border-border-subtle px-3 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-text-muted">
                {compositeMode ? "Autorité /100" : "Indice d'autorité"}
              </th>
            </tr>
          </thead>
          <tbody>
      ...
      {!compositeMode && (
        <p className="mt-3 text-[11px] font-light leading-snug text-text-muted">
          Le classement suit la <span className="font-medium">qualité d&apos;autorité</span>, pas
