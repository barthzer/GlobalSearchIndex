...
interface AnalysisVerificationProps {
  url: string;
  email: string;
  /** Lead capturé à l'onboarding → dérive le compte prospect après vérification. */
  lead?: OnboardingLead | null;
  /** Analyse terminée → redirection dashboard. `reused` = analyse déjà utilisée
   *  (verify-code a rendu un rapport existant), pour afficher le message idoine. */
  onComplete: (reused?: boolean) => void;
  /** Abandon de la vérification → retour à la landing. */
  onClose: () => void;
  /** Ouvre la modale expert (état plafond + upsell). */
  onExpertClick?: () => void;
}
 ...
 */
export default function AnalysisVerification({ url, email, lead, onComplete, onClose, onExpertClick }: AnalysisVerificationProps) {
  const { loginAsProspect } = useAccount();
  const [digits, setDigits] = useState<string[]>(Array(CODE_LENGTH).fill(""));
  const [checking, setChecking] = useState(false);
  const [verified, setVerified] = useState(false);
  const [confirmResend, setConfirmResend] = useState(false);
  // Plafond atteint (verify-code → cap_reached) : remplace la saisie du code par
  // l'état plafond + upsell expert. Réutilise la coquille de la modale existante.
  ...
  // verify-code a rendu un rapport DÉJÀ existant (status 'ready', 1 gratuite/email
  // déjà consommée) : on le mémorise pour l'annoncer au dashboard (« déjà utilisé »).
  ...
  const validate = useCallback(() => {
    ...
    setError("");
    // Vérification RÉELLE : POST /public/verify-code { email, code }.
    setChecking(true);
    (async () => {
      try {
        const result = await verifyPublicCode(email, digits.join(""));
        if (result.status === "cap_reached") {
          setChecking(false);
          setCapReached(true);
          return;
        }
        // Succès : REMPLACEMENT atomique de la session prospect. clear (purge
        // token + cache d'analyse de l'ancien parcours) PUIS set (nouveau token +
        // projectId) — aucun reliquat de l'analyse précédente ne peut subsister.
        clearProspectSession();
        // status 'ready' = rapport existant (analyse gratuite déjà utilisée par cet
        // email) ; 'processing' = analyse fraîche lancée. On le retient pour le message.
        reusedRef.current = result.status === "ready";
        setProspectSession({ token: result.token, projectId: result.projectId });
        if (lead) loginAsProspect(accountFieldsFromLead(lead));
        setChecking(false);
        setVerified(true);
      } catch (e) {
        // 401 (ou autre) : message d'erreur existant, on vide + refocus.
        setChecking(false);
  ...
  }, [complete, checking, expired, email, digits, lead, loginAsProspect]);
  ...
  function resend() {
    ...
    setError("");
    inputsRef.current[0]?.focus();
    // Renvoi RÉEL : POST /public/request-code { email, url } (best-effort neutre).
    requestPublicCode(email, url);
  }
      ...
      <div className="fixed inset-0 z-50 grid grid-cols-1 bg-white md:grid-cols-2">
        ...
        <div className="relative flex items-center justify-center bg-white px-6 py-12 md:px-10">
          ...
          <div className="w-full max-w-[380px]">
            {capReached ? (
              // État PLAFOND ATTEINT — réutilise la coquille (icône pink, classes de
              // titre/texte) + le bouton « Parler à un expert ». Wording neutre.
              <div style={{ animation: "fade-up 300ms var(--ease-expo) both" }}>
                ...
                <h2 className="text-[22px] font-medium tracking-[-0.4px] text-text-primary">
                  Analyse gratuite indisponible pour le moment
                </h2>
                <p className="mt-2 text-[14px] font-light leading-relaxed text-text-muted">
                  Revenez un peu plus tard, ou parlez à un expert dès maintenant pour obtenir votre bilan.
                </p>
                <div className="mt-6">
                  <Button variant="primary" fullWidth onClick={() => onExpertClick?.()}>
                    Parler à un expert
