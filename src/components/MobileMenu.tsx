...
export default function MobileMenu({ onClose, onExpertClick, activeTab, onTabChange }: MobileMenuProps) {
  ...
  // Vues du dashboard (sans l'accueil rendu en icône).
  const viewTabs = tabsForRole(isAdmin).filter((t) => !t.iconOnly);
      ...
      <div className="flex flex-col gap-1.5">
        {viewTabs.map((t, i) => (
          <button
            ...
            className={`flex items-center justify-between rounded-xl px-4 py-3.5 text-[16px] font-medium transition-colors duration-200 ${
              activeTab === t.key ? "bg-bg-card text-text-primary" : "text-text-secondary hover:bg-bg-card"
            }`}
            style={{ animation: `fade-up 300ms var(--ease-expo) ${80 + i * 40}ms both` }}
          >
            {t.label}
            {activeTab === t.key && (
